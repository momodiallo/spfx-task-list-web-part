    ...
    TaskListViewModel.prototype.DisplayListView = function (web, listId, viewId, userColl) {
        ...
        web.lists.getById(listId).views.getById(viewId).fields.get().then(function (val) {
            val.Items.forEach(function (element) {
                switch (element) {
                    case "MM_x0020_Due_x0020_Date":
                        element = strings.DueDate;
                        break;
                    case "DueDate":
                        element = strings.DueDate;
                        break;
                    case "Goedkeuring_budget_aanvragen":
                        element = strings.Goedkeuring;
                        break;
                    case "OpmerkingBudgetBeheerder":
                        element = strings.Opmerking;
                        break;
                    case "Kostenplaats_x003_kostenplaatsv":
                        element = strings.Kostenplaats;
                        break;
                    case "Status_x0020_WPLU":
                        element = strings.StatusWPLU;
                        break;
                    case "Terugkoppeling_x0020_CO":
                        element = strings.TerugkoppelingCO;
                        break;
                    case "AssignedTo":
                        element = strings.AssignedTo;
                        break;
                }
                var vh = {
                    Title: element,
